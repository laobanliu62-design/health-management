...
- **@mention must use full Matrix ID** (with domain) — run `echo matrix-local.hiclaw.io:18080` to get it. Never write `matrix-local.hiclaw.io:18080` literally in a message
...
- **MinIO writable paths** — you can only write to `hiclaw/hiclaw-storage/agents//` (your workspace) and `hiclaw/hiclaw-storage/shared/` (collaboration). All other paths will return 403.
